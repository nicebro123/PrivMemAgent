...
class ContextMinimizer:
    ...
    FACT_PATTERNS = (
        ...
        r"\b(?:now|currently|changed|updated|no longer|instead|remember|forget)\b",
    ...
    def minimize(
        ...
        if not self.config.enabled or len(units_text) <= 1:
        ...
        if not selected:
            selected = [max(units, key=lambda unit: (unit.score, -unit.token_count))]
